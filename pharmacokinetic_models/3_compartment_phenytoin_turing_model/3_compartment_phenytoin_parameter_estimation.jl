...
# ╔═╡ 1339e31c-a1dd-11f1-bd90-d715d0e1a606
begin
...
# ╔═╡ 81c259ff-4de4-4cda-9ecd-e3a4985f4372
df = CSV.read("phenytoin_mock_data.csv", DataFrame)

# ╔═╡ 3374cd26-2848-49e6-9692-bbc20356217b
function non_linear_compartment_model!(dM, M, p ,t)
	M1, M2, M3 = M
	Q2, Q3, V1, V2, V3, Vmax, Km, Kp2, Kp3 = p
	C1 = M1/V1
	elim = (Vmax*C1)/(Km+C1)
	dM[1] = (Q2/(V2*Kp2))*M2 + (Q3/(V3*Kp3))*M3 - (Q2/V1)*M1 - (Q3/V1)*M1 - elim
	dM[2] = (Q2/V1)*M1 - (Q2/(V2*Kp2))*M2
	dM[3] = (Q3/V1)*M1 - (Q3/(V3*Kp3))*M3
end

# ╔═╡ ce1584fc-e890-49c9-b677-26fdb5a06025
begin
	# Mock Data
	initial_dose = 300.0
	observed_central_concentrations = df.concentration_mgL
	times = df.time_hours
	# Constant baseline problem to use for remake
	const dummy_prob = ODEProblem(non_linear_compartment_model!, [1.0, 0.0, 0.0], (0.0, 48.0), zeros(9))
end

# ╔═╡ 68697d2b-c2d4-428c-a276-4143a3bc8159
@model function fit_parameters(observed_data, times)
	# Parameters
	Q2 ~ LogNormal(log(1.5),0.5)
	Q3 ~ LogNormal(log(0.5),0.5)
	V1 ~ LogNormal(log(5),0.02)
	V2 ~ LogNormal(log(10),0.5)
	V3 ~ LogNormal(log(30),0.5)
	Vmax ~ LogNormal(log(18),0.5)
	Km ~ LogNormal(log(5.0),0.5)
	Kp2 ~ LogNormal(log(1.5),0.5)
	Kp3 ~ LogNormal(log(8),0.5)

	# Error
	sigma ~ Exponential(1.0)
	
	p = [Q2, Q3, V1, V2, V3, Vmax, Km, Kp2, Kp3]
	u0 = [initial_dose, 0, 0]

	# Remake model using the dummy and new u0 and parameters
	
	prob_remade = remake(dummy_prob, u0 = u0, p = p)
	sol = solve(prob_remade, Rodas5P(autodiff=ADTypes.AutoFiniteDiff()), saveat = 0:1.0:48)

	# If any volume or rate constant drops too close to zero or goes negative,
    # tell Turing this space is mathematically impossible and return immediately.
    if V1 < 0.01 || V2 < 0.01 || V3 < 0.01 || Km < 0.01 || Vmax < 0.0
        Turing.@addlogprob! -Inf
        return
    end
	
	# Safety check to make sure the math didn't break
    if sol.retcode != ReturnCode.Success
        Turing.@addlogprob! -Inf
        return
    end

	# Extract the curve for the central compartment
	
    central_predictions_mass = sol[1, :]
	central_predictions_concentration = central_predictions_mass ./ V1
    observed_data ~ arraydist(Normal.(central_predictions_concentration, sigma))
    
end

# ╔═╡ c00684be-6187-498e-88a4-dce8df4936ba
begin
	rng = Xoshiro(42)
	experiment_instance = fit_parameters(observed_central_concentrations,times)
	chain = sample(rng, experiment_instance, NUTS(0.95), 1000)
end

# ╔═╡ 89303591-f67c-413f-b2be-b8ccbd5a53da
begin
	# 1. Define your true parameters explicitly
	p_true = [1.5, 0.5, 5.0, 10.0, 30.0, 18.0, 5.0, 1.5, 8.0]
	u0_true = [initial_dose, 0.0, 0.0]
	
	# 2. Solve the ODE directly on your time grid
	prob_true = remake(dummy_prob, u0 = u0_true, p = p_true)
	sol_true = solve(prob_true, Rodas5P(), saveat = times)
	
	# 3. Calculate true concentration
	true_concentration = sol_true[1, :] ./ 5.0
end


# ╔═╡ edebf2fe-6a4c-4911-8c40-364df1153ec6
begin
	  # Find the index of the iteration with the highest log-posterior density i.e. the best fit
    best_idx = argmax(chain[:log_density])
    
    best_Q2   = chain[:Q2][best_idx]
    best_Q3   = chain[:Q3][best_idx]
    best_V1   = chain[:V1][best_idx]
    best_V2   = chain[:V2][best_idx]
    best_V3   = chain[:V3][best_idx]
    best_Vmax = chain[:Vmax][best_idx]
    best_Km   = chain[:Km][best_idx]
    best_Kp2  = chain[:Kp2][best_idx]
    best_Kp3  = chain[:Kp3][best_idx]
    
	best_ps = [best_Q2, best_Q3, best_V1, best_V2, best_V3, best_Vmax, best_Km, best_Kp2, best_Kp3]	
end

# ╔═╡ ee984a3f-df82-4a53-af88-85768677fa48
begin
	# Best Values
	println("Q2: ", best_Q2)
	println("Q3: ", best_Q3)
	...
	println("Q2: ", best_Q2)
	println("Q3: ", best_Q3)
	println("Vmax: ", best_Vmax)
	println("Km: ", best_Km)
	println("Kp2: ", best_Kp2)
	println("Kp3: ", best_Kp3)
	println("sigma error: ", chain[:sigma][best_idx])
	# R-hats
	println("Q2 R-hat: ", rhat(chain[:Q2]))
	println("Q3 R-hat: ", rhat(chain[:Q3]))
	println("V1 R-hat: ", rhat(chain[:V1]))
	println("V2 R-hat: ", rhat(chain[:V2]))
	println("V3 R-hat: ", rhat(chain[:V3]))
	println("Vmax R-hat: ", rhat(chain[:Vmax]))
	println("Km R-hat: ", rhat(chain[:Km]))
	println("Kp2 R-hat: ", rhat(chain[:Kp2]))
	println("Kp3 R-hat: ", rhat(chain[:Kp3]))
	println("sigma error R-hat: ", rhat(chain[:sigma]))

end

# ╔═╡ bba505e6-9764-4969-af03-b2f7bb806dd4
begin
	best_u0 = [initial_dose,0,0]
	best_tspan = (0,48)
	best_prob = remake(dummy_prob, u0 = best_u0, tspan = best_tspan, p = best_ps)
	best_sol = solve(best_prob, Rodas5P(autodiff=ADTypes.AutoFiniteDiff()), abstol = 1e-8, reltol = 1e-8, saveat = 0:1.0:48)
	blood_mass = best_sol[1, :]
	blood_concentrations = blood_mass ./ best_V1
end

# ╔═╡ 22d377a8-e717-4f0a-a142-48c4faf9dc56
    p1 = scatter(times, observed_central_concentrations, 
        label="Observed Data (Lab)", 
        xlabel="Time (hr)", 
        ylabel="Concentration (mg/L)", 
        color=:red, 
        markersize=6)

# ╔═╡ 168c85d3-4a8e-46e0-bb5a-d6ac8deeb267
p2 = plot(p1, times, blood_concentrations, 
	  idxs = 1,
    label = "Turing Fitted Non-Linear Model", 
    colour=:blue, 
    lw = 2
    )
...
manifest_format = "2.0"
project_hash = "40a76fb0e70e76169b5bc1c17d32474b99561fab"
...
git-tree-sha1 = "2f879da24f01b7ee525dac2292654d2fc2b19dd8"
...
git-tree-sha1 = "749fc83e38cc65a1d47c1f7929d1bf84a44cfec4"
...
git-tree-sha1 = "6e0bf969e0f51bc5778dc33793be061aa6be3dac"
...
git-tree-sha1 = "c5f9ff04266b1b8936776b1465f910e917e7f4f6"
