...
# ╔═╡ 60d33871-4c35-4633-a7f6-6e0a629f4272
begin
...
# ╔═╡ 3deb95bf-df01-49a0-b487-5a9bf7233c02
...
# ╔═╡ 7fd441d5-86cc-497c-8e9a-0cf38d3b52c6
begin
	V1_true   = 5.0     # Central Volume (L)
	V2_true   = 10.0    # Peripheral Volume 2 (L)
	V3_true   = 30.0    # Deep Tissue Volume 3 (L)
	Q2_true   = 1.5     # Intercompartmental Flow 2 (L/hr)
	Q3_true   = 0.5     # Intercompartmental Flow 3 (L/hr)
	Vmax_true = 18.0    # Max elimination velocity (mg/hr)
	Km_true   = 5.0     # Michaelis constant (mg/L)
	Kp2_true  = 1.5     # Partition coefficient 2
	Kp3_true  = 8.0     # Partition coefficient 3 (highly lipophilic fat)
end

# ╔═╡ 7bc4b442-4baf-4a76-85ed-ba56ad0ee382
begin
	true_params = [Q2_true, Q3_true, V1_true, V2_true, V3_true, Vmax_true, Km_true, Kp2_true, Kp3_true]
	initial_dose = 300
	u0 = [initial_dose, 0, 0]
	tspan = (0, 48)
	t_steps = collect(0:1.0:48)
end

# ╔═╡ ccb2e2bf-fb6c-494c-b327-7d0df2b3d9aa
prob = ODEProblem(non_linear_compartment_model!, u0, tspan, true_params)

# ╔═╡ 46485012-a47d-4898-ba49-11bb85426a19
sol_perfect = solve(prob, Rodas5P(), saveat = t_steps)

# ╔═╡ c6e064c2-5066-4258-a3ab-46980bd21ec0
true_central_mass = sol_perfect[1, :]

# ╔═╡ e0ac9160-1e55-4e7d-bea4-cd3dd9e75cf0
true_concentration = true_central_mass ./ V1_true

# ╔═╡ a0ce38d0-a301-47da-9f20-ac2394fd7636
begin
	Random.seed!(42)   # Lock random numbers so your data is reproducible
	sigma_true = 0.25  # 0.25 mg/L of analytical measurement noise
	observed_concentration = true_concentration .+ randn(length(t_steps)) .* sigma_true # Add variation such that it follows a normal distribution
	# Structural check: ensure noise didn't accidentally push concentration below zero
	observed_concentration = max.(observed_concentration, 0.0)
end

# ╔═╡ d12a6333-5ec7-4cf3-8a91-92a1da9992d2
mock_df = DataFrame(
    time_hours = t_steps,
    concentration_mgL = observed_concentration
)

# ╔═╡ b8c36b7e-1909-4327-92f8-2025443850f8
begin
	CSV.write("mock_3_compartment_model_data.csv", mock_df)
	println("Mock dataset successfully generated and saved to 'mock_3_compartment_model_data.csv'!")
end
...
manifest_format = "2.0"
project_hash = "e65345a0eed64306aad66fa5575b73579951a20e"
...
git-tree-sha1 = "fd2476993d84cedaaf6a03cf1a291d9acc18f70e"
