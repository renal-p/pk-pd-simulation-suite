...
# ╔═╡ e2c65074-9b80-11f1-ad85-01929019b822
begin
...
# ╔═╡ d5019cd0-a775-4c31-bcd8-3424bab23752
function allosteric_modulator_binding!(du, u, p, t)
	LR, RM, LRM = u
	L_total, R_total, M_total, k_on_L, k_off_L, k_on_M, k_off_M, k_on_LR, k_off_LR, k_on_RM, k_off_RM = p
	L = L_total-LR-LRM # [L] = L_total - [LR] - [LRM] i.e. free ligand
	R = R_total-LR-RM-LRM # [R] = R_total - [LR] - [RM] - [LRM], i.e. unbound receptor
	M = M_total-RM-LRM # [M] = M_total - [RM] - [LRM], i.e. free antagonist
	v1 = k_on_L*L*R
	v2 = k_off_L*LR
	v3 = k_off_LR*LRM
	v4 = k_on_LR*LR*M
	...
	v7 = k_off_RM*LRM
	v8 = k_on_RM*RM*L
	du[1] = v1 + v3 - v2 - v4 # d[LR]/dt
	du[2] = v5 + v7 - v6 - v8 # d[RM]/dt
	du[3] = v4 + v8 - v3 -v7 # d[LRM]/dt	
end

# ╔═╡ 567c6b82-aeb1-4847-896c-fae2b4ed945d
u0 = [0.0, 0.0, 0.0] # [LR], [RM], [LRM]

# ╔═╡ 11c01812-a9fa-4ff0-b293-597003605bf5
tspan = (0,2) # 2 Minute period

# ╔═╡ d64c8951-92e3-4f23-b176-03039c102648
@bind M_conc Slider(0:1.0:100, default = 30.0, show_value=true)

# ╔═╡ 4ecb262f-fd03-4003-9e1c-1c5b896b8824
p = (
    # Total System Injected Concentrations (in nM)
    L_total = 50.0,    # Strong saturating dose of target ligand
    R_total = 10.0,    # Baseline receptor pool expression 
    M_total = M_conc,    # Active dose of allosteric modulator
    
    # Baseline Orthosteric Ligand Kinetics
    k_on_L = 0.1,      # Diffusion association rate (nM⁻¹ min⁻¹)
    k_off_L = 0.5,     # Baseline dissociation rate (min⁻¹). Kd_L = 5.0 nM
    
    # Baseline Allosteric Modulator Kinetics
    k_on_M = 0.05,     # Modulator association rate (nM⁻¹ min⁻¹)
    k_off_M = 0.25,    # Modulator dissociation rate (min⁻¹). Kd_M = 5.0 nM
    
    # Cooperatively Enhanced Convergent Pathways (Positive Allosteric Modulator, α = 5)
    # Pathway A: M binding to an already occupied LR complex
    k_on_LR = 0.5,     # Enhanced association rate (k_on_L * α)
    k_off_LR = 0.5,    # Dissociation remains steady
    
    # Pathway B: L binding to an already occupied RM complex
    k_on_RM = 0.5,     # Enhanced association rate (k_on_M * α)
    k_off_RM = 0.5    # Dissociation remains steady
)

# ╔═╡ e13b441f-b974-4452-9041-98f4031472a1
# Thermodynamics Validation Cell
let
    # 1. Calculate the 4 individual equilibrium constants (Kd = koff / kon)
    Kd_L  = p.k_off_L  / p.k_on_L
    Kd_M  = p.k_off_M  / p.k_on_M
    Kd_LR = p.k_off_LR / p.k_on_LR
    Kd_RM = p.k_off_RM / p.k_on_RM
    
    # 2. Check both paths around the cycle
    clockwise_path = Kd_L * Kd_LR
    counter_clockwise_path = Kd_M * Kd_RM
    
    # 3. Use approximate equality to account for minor floating-point math rounding
    if isapprox(clockwise_path, counter_clockwise_path, atol=1e-6)
        "✅ Thermodynamics Valid: Detailed balance holds ($clockwise_path == $counter_clockwise_path)"
    else
        "❌ THERMODYNAMIC VIOLATION: The system forms a perpetual energy leak! Clockwise ($clockwise_path) ≠ Counter-Clockwise ($counter_clockwise_path)"
    end
end


# ╔═╡ 8e70c5bb-af2c-493f-bf30-837870fdc21f
prob = ODEProblem(allosteric_modulator_binding!, u0, tspan, p)

# ╔═╡ bcb5fa33-d3bc-48c2-bb76-2e180f0b23db
sol = solve(prob, Rosenbrock23(), abstol = 1e-8, reltol = 1e-8)

# ╔═╡ 1d311272-4fc0-44d2-ac47-2f3aca1e2647
plot(sol,
	idxs = [1,2,3],
	labels = ["LR" "RM" "LRM"],
	xlabel = "Time (min)",
	ylabel = "Concentration (nM)",
	title = "Ternary Allosteric Binding",
	lw = 2)
...
manifest_format = "2.0"
project_hash = "36df02cf24f4ffcc25b3ac8ea3deebed11e57430"
